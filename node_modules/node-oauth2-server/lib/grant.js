...
function usePasswordGrant(done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    ...
    if (user.last_update == null) {
      return done(error('invalid_grant', 'Password most be update'))
    }
...
function sendResponse(done) {
  ...
  /*   if (!this.req.body.last_update) {
      console.log('Last_update= ' +this.req.body)
      response = {}
      //response.error = 'Password most be update'
      response.error = this.res.body

      this.res.status(400).jsonp(response)
    } else {
      this.res
