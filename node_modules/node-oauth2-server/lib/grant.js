...
function usePasswordGrant(done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    if (err) return done(error('server_error', false, err));
    //This error handling was deleted to allow a better response on the front end
     if (!user) {
      return done(error('invalid_request', 'User credentials are invalid'))
    } 
    self.user = user;
    done();
  });
...
function sendResponse(done) {
  ...
  //Custom error handling on the response 
  if (this.user != undefined) {
    if (this.user.last_update === null) {
